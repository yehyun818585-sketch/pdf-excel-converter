...
- **게이트 B** — OCR 결과가 50자 이상이면 그 텍스트를 채택하고 AI Vision을 **건너뜀**
...
| **Claude API** (Anthropic) | 형식이 제각각인 **증빙 7종의 유형 판별과 표준 양식의 필드 추출**이 핵심 → 긴 문맥의 문서 이해에 활용 (**출력을 그대로 신뢰하지 않고** 숫자는 코드가 재계산) |
